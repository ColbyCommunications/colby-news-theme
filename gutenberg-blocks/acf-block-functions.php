...
function innerBlocks( $args = array() ) {
	$default_args = array(
...
function query_from_fields( array $user_fields = array(), $has_pagination = false, $post_id = null ) {
	...
	$new_args            = array();
	$customizable_fields = array( 'post_type', 'posts_per_page', 'order', 'orderby', 'post_status', 'ignore_sticky_posts' );
	...
	if ( $post_format && $post_format !== 'all' ) {
		...
		if ( $post_format === 'standard' ) {
			...
			$post_format_query['terms']    = array( "post-format-$post_format" );
			$post_format_query['operator'] = 'IN';
		}
		$tax_query_master = array( $post_format_query );
	} else {
		...
		foreach ( $taxonomy_terms as $term_object ) {
			...
			if ( is_object( $term_object ) ) {
				if ( ! array_key_exists( $term_object->taxonomy, $taxonomies ) ) {
					$taxonomies[ $term_object->taxonomy ] = array();
				}
	...
	if ( ! $include_current && $post_id ) {
		$posts_not_in = is_array( $posts_not_in ) ? array_merge( $posts_not_in, array( $post_id ) ) : array( $post_id );
	}
...
function nc_get_excerpt( $post, $trim = false, array $custom_fields = array() ) {
	$post = get_post( $post );
	if ( empty( $post ) ) {
		return '';
	}
...
function related_posts( $post_id, $item_count = 3, array $tags = array() ) {
	...
	$query_args = array(
		...
		'tax_query'           => array(
			...
			array(
				'taxonomy' => 'category',
				'field'    => 'slug',
				'terms'    => array( $primary_category->slug ),
			),
		),
	);
...
function teaser_list( array $posts, bool $is_preview = false, array $show_fields = array(
	...
	$show_fields_arrays = array();

	foreach ( $show_fields as $field_def ) {
		if ( is_string( $field_def ) ) {
			$show_fields_arrays[ $field_def ] = array(
				'display' => true,
			);
		}
	...
	foreach ( $posts as $post ) {
		if ( ! is_array( $post ) ) {
			...
			$post_formats = wp_get_post_terms( $post->ID, 'post_format', array( 'fields' => 'names' ) );
            ...
            <img
                srcset="
                    https://news.colby.edu/cdn-cgi/image/width=1080,quality=30,format=auto/https://news.colby.edu{$image_path} 1080w, 
                    https://news.colby.edu/cdn-cgi/image/width=300,quality=30,format=auto/https://news.colby.edu{$image_path} 300w, 
                    https://news.colby.edu/cdn-cgi/image/width=1024,quality=30,format=auto/https://news.colby.edu{$image_path} 1024w,
                    https://news.colby.edu/cdn-cgi/image/width=600,quality=30,format=auto/https://news.colby.edu{$image_path} 600w,
                    https://news.colby.edu/cdn-cgi/image/width=1536,quality=30,format=auto/https://news.colby.edu{$image_path} 1536w,
                    https://news.colby.edu/cdn-cgi/image/width=100,quality=30,format=auto/https://news.colby.edu{$image_path} 100w,
                    https://news.colby.edu/cdn-cgi/image/width=540,quality=30,format=auto/https://news.colby.edu{$image_path} 540w,
                    https://news.colby.edu/cdn-cgi/image/width=100,quality=30,format=auto/https://news.colby.edu{$image_path} 100w,
                    https://news.colby.edu/cdn-cgi/image/width=800,quality=30,format=auto/https://news.colby.edu{$image_path} 800w,
                    https://news.colby.edu/cdn-cgi/image/width=640,quality=30,format=auto/https://news.colby.edu{$image_path} 640w,
                    https://news.colby.edu/cdn-cgi/image/width=320,quality=30,format=auto/https://news.colby.edu{$image_path} 320w,
                    https://news.colby.edu/cdn-cgi/image/width=1090,quality=30,format=auto/https://news.colby.edu{$image_path} 1090w,
                    https://news.colby.edu/cdn-cgi/image/width=400,quality=30,format=auto/https://news.colby.edu{$image_path} 400w,
                    https://news.colby.edu/cdn-cgi/image/width=1600,quality=30,format=auto/https://news.colby.edu{$image_path} 1600w,
                "
        ...
        EOD;
			...
			if ( in_array( 'category', $show_fields ) ) {
				$teaser['superhead'] = array(
					'title' => $primary_category_name,
...
function post_list_slider( $posts, $is_preview = false ) {
	...
	return Timber::compile(
		get_blocks_twig_directory( '/sliding-teasers.twig' ),
		array(
			'teasers'    => $teasers,
...
function teaser_pair( $posts, $is_preview = false, $show_fields = array(
	...
	return Timber::compile(
		get_blocks_twig_directory( '/teaser-pair.twig' ),
		array(
			'teasers'    => $teasers,
			'is_preview' => $is_preview,
		)
...
// Algolia personalized results
function algolia_personalized_results_block( $block, $content = '', $is_preview = false, $post_id = 0 ) {
	if ( is_admin() ) {
		echo 'Algolia Personalized Section';
	}
	...
	if ( function_exists( 'get_fields' ) ) {
		...
		$fields_from_block = is_array( $fields_from_block ) ? $fields_from_block : array();
...
function external_post_teaser_args( $post, $args = array() ) {
...
function basic_post_teaser_args( $post, $args = array() ) {
	if ( ! empty( $args['show_description'] ) ) {
		$blurb = nc_get_excerpt( $post );
	} else {
		$blurb = null;
...
function external_post_list( $block, $content = '', $is_preview = false, $post_id = 0 ) {
	...
	if ( is_array( $post_results ) && count( $post_results ) > 0 ) {
		foreach ( $post_results as $index => $post ) {
			...
			$item_args = external_post_teaser_args( $post, array( 'show_description' => $show_description ) );
...
function related_posts_block( $block, $content = '', $is_preview = false, $post_id = 0 ) {
	...
	if ( $related_posts ) {
		...
		$teasers = array_map(
			function ( $post_item ) use ( $image_size ) {
		...
		Timber::render(
			get_blocks_twig_directory( '/related-stories.twig' ),
			array(
				'items' => $teasers,
				'float' => $align,
			)
...
function breaker_feature( $block, $content = '', $is_preview = false, $post_id = 0 ) {
	if ( function_exists( 'get_field' ) ) {
		...
		if ( $image ) {
            ...
            <img
                srcset="
                    https://news.colby.edu/cdn-cgi/image/width=1090,quality=30,format=auto/https://news.colby.edu{$image_path} 2160w, 
                    https://news.colby.edu/cdn-cgi/image/width=300,quality=30,format=auto/https://news.colby.edu{$image_path} 300w, 
                    https://news.colby.edu/cdn-cgi/image/width=1024,quality=30,format=auto/https://news.colby.edu{$image_path} 1024w,
                    https://news.colby.edu/cdn-cgi/image/width=150,quality=30,format=auto/https://news.colby.edu{$image_path} 150w,
                    https://news.colby.edu/cdn-cgi/image/width=600,quality=30,format=auto/https://news.colby.edu{$image_path} 600w,
                    https://news.colby.edu/cdn-cgi/image/width=1536,quality=30,format=auto/https://news.colby.edu{$image_path} 1536w,
                    https://news.colby.edu/cdn-cgi/image/width=2048,quality=30,format=auto/https://news.colby.edu{$image_path} 2048w,
                    https://news.colby.edu/cdn-cgi/image/width=80,quality=30,format=auto/https://news.colby.edu{$image_path} 80w,
                    https://news.colby.edu/cdn-cgi/image/width=100,quality=30,format=auto/https://news.colby.edu{$image_path} 100w,
                    https://news.colby.edu/cdn-cgi/image/width=800,quality=30,format=auto/https://news.colby.edu{$image_path} 800w,
                    https://news.colby.edu/cdn-cgi/image/width=640,quality=30,format=auto/https://news.colby.edu{$image_path} 640w,
                    https://news.colby.edu/cdn-cgi/image/width=320,quality=30,format=auto/https://news.colby.edu{$image_path} 320w,

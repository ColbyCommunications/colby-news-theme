const setUpSiteHeader = () => {
  ...
  const setUpModal = ({ menuId, openButtonClass, closeButtonId }) => {
    ...
    const focusTarget =
      focusTargetAncestor &&
      document
        .createTreeWalker(focusTargetAncestor, NodeFilter.SHOW_ELEMENT, {
          acceptNode: (node) =>
            node.tabIndex >= 0
    ...
    openButtons.forEach((button) =>
      ...
      */
        menu.addEventListener(
          'animationend',
          function openCleanup({ currentTarget, target }) {
    ...
    closeButton.addEventListener('click', () => {
      ...
      menu.addEventListener(
        'animationend',
        function closeCleanup({ currentTarget, target }) {
          if (target !== currentTarget) return;
    ...
    */
    ...
    class StickyEvents {
      ...
      constructor({
        container = document,
        enabled = true,
        stickySelector = '.sticky-events',
      } = {}) {
        this.container = container;
        this.observers = [];
        this.stickyElements = Array.from(
          this.container.querySelectorAll(stickySelector)
        );
      ...
      createHeaderObserver() {
        return new IntersectionObserver(
          ([record]) => {
            ...
            const stickyTarget = stickyParent.querySelector(
              this.stickySelector
            );
      ...
      fire(isSticky, stickyTarget, position) {
        ...
        stickyTarget.dispatchEvent(
          new CustomEvent(StickyEvents.CHANGE, {
            detail: { isSticky, position },
            bubbles: true,
          })
        );
        stickyTarget.dispatchEvent(
          new CustomEvent(
            isSticky ? StickyEvents.STUCK : StickyEvents.UNSTUCK,
            { detail: { isSticky, position }, bubbles: true }
          )
        );
        ...
        switch (className) {
          ...
          case ClassName.SENTINEL_BOTTOM:
            ...
            return {
              ...
              height: `${
                stickyElement.getBoundingClientRect().height + parentPadding
              }px`,
            };
        }
      }
      ...
      isSticking(stickyElement) {
        ...
        const difference = Math.round(
          Math.abs(stickyOffset - topSentinelOffset)
        );

        const topSentinelTopPosition = Math.abs(
          parseInt(window.getComputedStyle(topSentinel).getPropertyValue('top'))
        );
